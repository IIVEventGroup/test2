...
class OSTrack(nn.Module):
    ...
    def forward_head(self, cat_feature, gt_score_map=None, time_to_next=None):
        ...
        bs, Nq, C, HW = opt.size()
        opt_feat = opt.view(-1, C, self.feat_sz_s, self.feat_sz_s) #(bs,768,16,16)
...
def build_ostrack(cfg, settings, training=True):
    ...
    box_head = build_box_head(cfg, hidden_dim, settings.pred_next)
