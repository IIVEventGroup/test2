...
class CenterPredictor(nn.Module, ):
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16, freeze_bn=False, pred_next=0):
        ...
        # speed regress, speed of (x,y,w,h)
        self.pred_next = pred_next
        if self.pred_next:
            self.conv1_speed = conv(inplanes, channel, freeze_bn=freeze_bn)
            self.conv2_speed = conv(channel, channel // 2, freeze_bn=freeze_bn)
            self.conv3_speed = conv(channel // 2, channel // 4, freeze_bn=freeze_bn)
            self.conv4_speed = conv(channel // 4, channel // 8, freeze_bn=freeze_bn)
            self.conv5_speed = nn.Conv2d(channel // 8, 4, kernel_size=1)
    ...
    def cal_bbox(self, score_map_ctr, size_map, offset_map, speed_map, time_to_next, return_score=False):
        ...
        size = size_map.flatten(2).gather(dim=2, index=idx) #(bs,2,1)
        offset = offset_map.flatten(2).gather(dim=2, index=idx).squeeze(-1) #(bs,2)
    ...
    def get_score_map(self, x):
        ...
        score_map_ctr = self.conv5_ctr(x_ctr4) #(bs,1,16,16)
        ...
        score_map_offset = self.conv5_offset(x_offset4) #(bs,2,16,16)
        ...
        score_map_size = self.conv5_size(x_size4) #(bs,2,16,16)

        # speed branch
        score_map_speed = None
        if self.pred_next:
            x_speed1 = self.conv1_speed(x)
            x_speed2 = self.conv2_speed(x_speed1)
            x_speed3 = self.conv3_speed(x_speed2)
            x_speed4 = self.conv4_speed(x_speed3)
            score_map_speed = self.conv5_speed(x_speed4) #(bs,4,16,16)


        return _sigmoid(score_map_ctr), _sigmoid(score_map_size), score_map_offset, score_map_speed
...
def build_box_head(cfg, hidden_dim, pred_next):
    ...
    if cfg.MODEL.HEAD.TYPE == "MLP":
        ...
        center_head = CenterPredictor(inplanes=in_channel, channel=out_channel,
                                      feat_sz=feat_sz, stride=stride, pred_next=pred_next)
        return center_head
    else:
