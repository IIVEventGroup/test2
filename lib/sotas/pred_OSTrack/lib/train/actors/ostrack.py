...
class OSTrackActor(BaseActor):
    ...
    def forward_pass(self, data):
        ...
        self.time_to_next = data['time_to_next'].view(-1,1)
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        gt_gaussian_maps = generate_heatmap(gt_dict['search_anno'], self.cfg.DATA.SEARCH.SIZE, self.cfg.MODEL.BACKBONE.STRIDE) #(bs,16,16)
        ...
        if self.settings.pred_next:
            ...
            try:
                pred_next_giou_loss, pred_next_iou = self.objective['giou'](pred_next_bbox_vec, gt_next_bbox_vec)  # (BN,4) (BN,4)
            except:
                pred_next_giou_loss, pred_next_iou = torch.tensor(0.0).cuda(), torch.tensor(0.0).cuda()
        ...
        if return_status:
            ...
            status = {"Loss/total": loss.item(),
                      ...
                      "IoU": mean_iou.item()}
            if self.settings.pred_next:
                status['pred_next/giou'] = pred_next_giou_loss.item()
                status['pred_next/l1'] = pred_next_l1_loss.item()
                status['pred_next/IoU'] = pred_next_iou.detach().mean().item()
